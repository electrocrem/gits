...
Logseq, Qt/Dolphin (Kvantum), сборщик темы Telegram, обои через awww (`gits-wall --pick`, в том числе анимированные GIF, ~2% одного ядра; выбор запоминается), а также `gits-doctor` — отчёт о состоянии системы и опциональная настройка снапшотов btrfs.
