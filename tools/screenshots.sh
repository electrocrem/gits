...
echo "desktop";   shot "$OUT/desktop.jpg"
...
echo "control panel"; popup control "$OUT/control-panel.jpg" 930 40 350 700
echo "player";        popup media   "$OUT/player.jpg"        520 40 360 490
echo "mixer";         popup mixer   "$OUT/mixer.jpg"         500 40 380 300
echo "notifications"; popup notify  "$OUT/notifications.jpg" 900 40 380 400
...
echo "osd"; gits-osd volume 64; sleep 0.7; shot "$OUT/osd.jpg" 430 590 420 140; sleep 1.2
...
echo "terminal"; read -r x y w h < <(WAIT=9 throwaway gits-shot-term env GITS_BANNER_FAST=1 zsh -i); shot "$OUT/terminal.jpg" $x $y $w $h
echo "nvim dashboard"; read -r x y w h < <(WAIT=8 throwaway gits-shot-nvim nvim); shot "$OUT/nvim-dashboard.jpg" $x $y $w $h
echo "done: $OUT"
